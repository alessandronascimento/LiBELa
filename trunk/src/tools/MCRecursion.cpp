...
//!TODO: this class should have a sampling method receiving two parameters W(b,T), where b is a constant
//! and T is the effective temperature. The strategy follows the idea proposed by Li and Scheraga (1989) in
//! Chem Phys Lett.
...
void MC::run(Grid* Grids, Mol2* RefLig, Mol2* Lig, vector<vector<double> > xyz, PARSER* Input, double b, double T){
    this->xyz = xyz;

    this->MaxMin.push_back(0);
    this->MaxMin.push_back(0);
    this->MaxMin.push_back(0);
    this->MaxMin.push_back(0);
    this->MaxMin.push_back(0);
    ...
    if (Input->eq_mode){
        ...
        while (eqcount <= Input->eq_steps){
            ...
            if (new_energy <= energy){
                ...
                p = this->Boltzmman_recursive(energy, new_energy, b, T);
        ...
        Writer->print_line();
        sprintf(info, "Equilibration done with %5d steps. Current system energy: %9.3f kcal/mol.", eqcount, energy);
        Writer->print_info(info);
        Writer->print_line();
        ...
        Writer->print_line();
        sprintf(info, "Finished MC simulation after %d accepted steps.", Input->number_steps);
        Writer->print_info(info);
        Writer->print_line();
        ...
        sprintf(info, "%10.3Lf %10.3Lf %10.3Lf", this->XSize, this->YSize, this->ZSize);
        Writer->print_info(info);
        Writer->print_line();
    }
}
...
void MC::run(Mol2* Rec, Mol2* RefLig, Mol2* Lig, vector<vector<double> > xyz, PARSER* Input){
/*
    this->xyz = xyz;
    int nReject = 0;
    ...
    if (Input->eq_mode){
        ...
        sprintf(info, "Starting Monte Carlo equilibration simulation with sweep size of %5d and %5d sweeps...", (ST , nsweeps));
        Writer->print_info(info);
        gzFile mc_output;
        mc_output = gzopen((Input->output + "_mc.dat.gz").c_str(), "w");
...
void MC::MaxMinCM(long double XCOM, long double YCOM, long double ZCOM, vector <long double> Max){
...
double MC::Boltzmman_recursive(double ene, double new_ene, double b, double t){
    double x = - (b-1.0)*(new_ene-ene) / (0.0019858775203792202*t); // Boltzmann constant k=0.0019858775203792202 kcal/(mol.K)
    return(exp(x));
}
